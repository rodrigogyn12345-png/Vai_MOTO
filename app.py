...
function br(v){return Number(v||0).toLocaleString('pt-BR',{style:'currency',currency:'BRL'});}
function esc(v){return String(v??'').replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]));}
function som(){if(!audioLiberado)return;try{const C=AudioContext||webkitAudioContext,ctx=new C();[0,350,700].forEach((t,i)=>{setTimeout(()=>{const o=ctx.createOscillator(),g=ctx.createGain();o.type='square';o.frequency.value=i%2?1000:750;g.gain.value=.28;o.connect(g);g.connect(ctx.destination);o.start();setTimeout(()=>o.stop(),220)},t)});setTimeout(()=>ctx.close(),1100)}catch(e){}}

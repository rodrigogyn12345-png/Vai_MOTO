...
from flask import Flask, request, jsonify, session, redirect, url_for, render_template_string, send_from_directory, make_response
...
def need(tipo=None):
    def deco(fn):
        ...
        def wrap(*a, **kw):
            u=user()
            if not u:
                return (jsonify(ok=False,erro="Faça login primeiro."),401) if request.path.startswith("/api/") else redirect(url_for("login"))
...
def vals(km):
    total = round(float(km) * PRECO_KM, 2)
...
def online_count():
    c=db()
    r=c.execute("""SELECT COUNT(*) n FROM usuarios_vai
        WHERE tipo='motorista' AND aprovado=1 AND online=1
        AND (last_seen IS NULL OR last_seen>?)""",(time.time()-ONLINE_TIMEOUT,)).fetchone()["n"]
    c.close(); return r

def geocode(addr, near_lat=None, near_lon=None):
    q=addr.strip()
    if not q: return None
    # Coordenadas digitadas continuam aceitas
    try:
        p=[x.strip() for x in q.replace(";",",").split(",")]
        la,lo=float(p[0]),float(p[1])
        if -90<=la<=90 and -180<=lo<=180:
            return la,lo,q
    except Exception:
        pass
    # Geocodificação gratuita pelo Nominatim
    try:
        params = quote(q + ", Brasil")
        req=Request(
            "https://nominatim.openstreetmap.org/search?format=jsonv2&limit=1&q="+params,
            headers={"User-Agent":"VAI_DE_MOTO/1.0"}
        )
...
<link rel="apple-touch-icon" href="/static/pwa/icone-passageiro-192.png">
<script>
...
let vaimotoInstallPrompt = null;
window.addEventListener('beforeinstallprompt', function(e) {
  e.preventDefault();
  vaimotoInstallPrompt = e;
  document.querySelectorAll('.install-app').forEach(function(b){ b.style.display='block'; });
});
async function instalarVAI_DE_MOTO(){
  if(!vaimotoInstallPrompt){
    alert('No Android/Chrome, abra o menu ⋮ e toque em “Adicionar à tela inicial” ou “Instalar app”.');
    return;
  }
  vaimotoInstallPrompt.prompt();
  await vaimotoInstallPrompt.userChoice;
  vaimotoInstallPrompt = null;
...
<select id="pagamento" onchange="pagUI()">
<option value="DINHEIRO">💵 Dinheiro — paga ao motorista no final</option>
<option value="PIX">📱 PIX — paga antecipado ao VAI_DE_MOTO</option>
</select>
<div id="pixAdmin" class="box" style="display:none">
  📱 <b>PIX para pagamento da corrida</b><br>
  Chave PIX: <b id="chavePixAdmin">62993903299</b><br>
  <small>Envie exatamente o valor mostrado acima. A corrida só será liberada depois que o pagamento for confirmado pelo administrador.</small>
</div>
...
const PIX_ADMIN='62993903299';
$('chavePixAdmin').textContent=PIX_ADMIN;
function pagUI(){
 const f=$('pagamento').value;
 $('pixAdmin').style.display=f==='PIX'?'block':'none';
 $('pagAviso').textContent=f==='DINHEIRO'
   ?'💵 Você paga ao motorista somente no final da corrida. A corrida será liberada imediatamente.'
   :'📱 PIX antecipado para o administrador. A corrida NÃO será enviada ao motorista até o PIX ser confirmado.';
}
...
async function solicitar(){
 ...
 try{
   ...
   $('msg').textContent=d.pagamento_status==='AGUARDANDO_PAGAMENTO'?'📱 Corrida #'+d.corrida.id+' criada. Faça o PIX para o administrador. Só será liberada após a confirmação.':'✅ Corrida #'+d.corrida.id+' enviada aos motoristas. Você paga ao motorista em DINHEIRO no final.';
   calc=null;carregar();
...
async function disponiveis(){
 ...
 '<br>💳 '+esc(c.pagamento)+(c.pagamento==='DINHEIRO'?'<br>💵 Recebimento do passageiro: DINHEIRO no final da corrida.':'<br>📱 PIX já pago ao administrador; não cobrar PIX do passageiro.')+'<a class="btn blue" target="_blank" href="https://www.google.com/maps/search/?api=1&query='+encodeURIComponent(c.latitude_partida+','+c.longitude_partida)+'">📍 IR PARA EMBARQUE</a>'+btn(c)+'</div>').join('')||'<div class="box">Nenhuma corrida pendente.</div>';
 }catch(e){}
}
...
<div class="card"><h2>📱 PIX do administrador</h2><div class="box">Chave para receber os PIX das corridas: <b>62993903299</b><br><small>Troque ADMIN_PIX_KEY no app.py pela chave PIX real antes de usar em produção.</small></div></div><div class="card"><h2>💸 Saques</h2><div id="saques">...</div></div></div>
<script>
function esc(x){return String(x??'').replace(/[&<>"']/g,m=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#039;'}[m]))}
async function load(){
...
def manifest():
    return send_from_directory(os.path.join(app.root_path, "static", "pwa"), "manifest.json", mimetype="application/manifest+json", max_age=0)

@app.route("/service-worker.js")
def service_worker():
    resp = make_response(send_from_directory(os.path.join(app.root_path, "static", "pwa"), "service-worker.js", mimetype="application/javascript"))
...
<p class="sub">R$ 2,00/km • App 9% • Motorista 91%</p><button class="btn yellow install-app" style="display:none" onclick="instalarVAI_DE_MOTO()">📲 INSTALAR VAI_DE_MOTO</button><a class="btn yellow" href="/login">🔐 ENTRAR</a><a class="btn green" href="/cadastro">📝 CADASTRAR</a></div></div>""")

...
async function solicitarCorrida(){
  ...
  try{
    const r=await fetch('/api/solicitar-corrida',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({
      ...
      distancia_km:calculoAtual.distancia_km,valor:calculoAtual.valor,
...
def api_solicitar_corrida():
    ...
    try:
        ...
        cur=con.execute("""
          INSERT INTO corridas_vai(passageiro_id,partida,destino,valor,status,motorista_id,latitude_partida,longitude_partida,latitude_destino,longitude_destino,distancia_km,preco_km,taxa_admin_percent,taxa_admin,valor_motorista,criada_em)
          VALUES(?,?,?,?,'PENDENTE',NULL,?,?,?,?,?,?,?,?,?,?)
        """,(u["id"],partida,destino,total,lat,lon,lat2,lon2,dist,PRECO_KM,TAXA_ADMIN*100,taxa,mot,time.time()))
        con.commit();rid=cur.lastrowid
...
def api_corridas_disponiveis():
    ...
    marcar_motoristas_expirados();con=conectar();rows=con.execute("""
